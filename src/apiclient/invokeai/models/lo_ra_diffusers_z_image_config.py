...
T = TypeVar("T", bound="LoRADiffusersZImageConfig")


@_attrs_define
class LoRADiffusersZImageConfig:
    ...
    Attributes:
        ...
        base (Literal['z-image']):  Default: 'z-image'.
        variant (None | ZImageVariantType):
    """
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        variant: None | str
        if isinstance(self.variant, ZImageVariantType):
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        base = cast(Literal["z-image"], d.pop("base"))
        if base != "z-image":
            raise ValueError(f"base must match const 'z-image', got '{base}'")

        def _parse_variant(data: object) -> None | ZImageVariantType:
            if data is None:
                return data
            try:
                if not isinstance(data, str):
                    raise TypeError()
                variant_type_0 = ZImageVariantType(data)

                return variant_type_0
            except (TypeError, ValueError, AttributeError, KeyError):
                pass
            return cast(None | ZImageVariantType, data)

        variant = _parse_variant(d.pop("variant"))

        lo_ra_diffusers_z_image_config = cls(
            key=key,
            hash_=hash_,
            path=path,
            file_size=file_size,
        ...
        lo_ra_diffusers_z_image_config.additional_properties = d
        return lo_ra_diffusers_z_image_config
