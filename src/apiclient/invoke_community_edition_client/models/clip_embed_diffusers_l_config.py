...
T = TypeVar("T", bound="CLIPEmbedDiffusersLConfig")


@_attrs_define
class CLIPEmbedDiffusersLConfig:
    """
    Attributes:
        ...
        variant (Literal['large']):  Default: 'large'.
    """
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        variant = cast(Literal["large"], d.pop("variant"))
        if variant != "large":
            raise ValueError(f"variant must match const 'large', got '{variant}'")

        clip_embed_diffusers_l_config = cls(
            key=key,
            hash_=hash_,
            path=path,
            file_size=file_size,
        ...
        clip_embed_diffusers_l_config.additional_properties = d
        return clip_embed_diffusers_l_config
