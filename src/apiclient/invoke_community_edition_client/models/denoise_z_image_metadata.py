...
from ..models.denoise_z_image_metadata_scheduler import DenoiseZImageMetadataScheduler
...
T = TypeVar("T", bound="DenoiseZImageMetadata")


@_attrs_define
class DenoiseZImageMetadata:
    """Run denoising process with a Z-Image transformer model + metadata.

    Attributes:
        id (str): The id of this instance of an invocation. Must be unique among all instances of invocations.
        type_ (Literal['z_image_denoise_meta']):  Default: 'z_image_denoise_meta'.
        ...
        scheduler (DenoiseZImageMetadataScheduler | Unset): Scheduler (sampler) for the denoising process. Euler is the
            default and recommended. Heun is 2nd-order (better quality, 2x slower). LCM works with Turbo only (not Base).
            Default: DenoiseZImageMetadataScheduler.EULER.
    """

    id: str
    type_: Literal["z_image_denoise_meta"] = "z_image_denoise_meta"
    ...
    scheduler: DenoiseZImageMetadataScheduler | Unset = DenoiseZImageMetadataScheduler.EULER
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        type_ = cast(Literal["z_image_denoise_meta"], d.pop("type"))
        if type_ != "z_image_denoise_meta":
            raise ValueError(f"type must match const 'z_image_denoise_meta', got '{type_}'")
        ...
        scheduler: DenoiseZImageMetadataScheduler | Unset
        if isinstance(_scheduler, Unset):
            scheduler = UNSET
        else:
            scheduler = DenoiseZImageMetadataScheduler(_scheduler)

        denoise_z_image_metadata = cls(
            id=id,
            type_=type_,
            metadata=metadata,
        ...
        denoise_z_image_metadata.additional_properties = d
        return denoise_z_image_metadata
