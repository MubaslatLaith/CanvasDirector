...
T = TypeVar("T", bound="TIFileSDXLConfig")


@_attrs_define
class TIFileSDXLConfig:
    """
    Attributes:
        ...
        format_ (Literal['embedding_file']):  Default: 'embedding_file'.
    ...
    format_: Literal["embedding_file"] = "embedding_file"
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        format_ = cast(Literal["embedding_file"], d.pop("format"))
        if format_ != "embedding_file":
            raise ValueError(f"format must match const 'embedding_file', got '{format_}'")

        base = cast(Literal["sdxl"], d.pop("base"))
        if base != "sdxl":
            raise ValueError(f"base must match const 'sdxl', got '{base}'")

        ti_file_sdxl_config = cls(
            key=key,
            hash_=hash_,
            path=path,
            file_size=file_size,
        ...
        ti_file_sdxl_config.additional_properties = d
        return ti_file_sdxl_config
