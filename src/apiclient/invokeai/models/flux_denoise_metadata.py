...
from ..models.flux_denoise_metadata_dype_preset import FLUXDenoiseMetadataDypePreset
from ..models.flux_denoise_metadata_scheduler import FLUXDenoiseMetadataScheduler
...
T = TypeVar("T", bound="FLUXDenoiseMetadata")


@_attrs_define
class FLUXDenoiseMetadata:
    """Run denoising process with a FLUX transformer model + metadata.

    Attributes:
        id (str): The id of this instance of an invocation. Must be unique among all instances of invocations.
        type_ (Literal['flux_denoise_meta']):  Default: 'flux_denoise_meta'.
        ...
        scheduler (FLUXDenoiseMetadataScheduler | Unset): Scheduler (sampler) for the denoising process. 'euler' is fast
            and standard. 'heun' is 2nd-order (better quality, 2x slower). 'lcm' is optimized for few steps. Default:
            FLUXDenoiseMetadataScheduler.EULER.
        ...
        dype_preset (FLUXDenoiseMetadataDypePreset | Unset): DyPE preset for high-resolution generation. 'auto' enables
            automatically for resolutions > 1536px. 'area' enables automatically based on image area. '4k' uses optimized
            settings for 4K output. Default: FLUXDenoiseMetadataDypePreset.OFF.
    ...
    id: str
    type_: Literal["flux_denoise_meta"] = "flux_denoise_meta"
    ...
    scheduler: FLUXDenoiseMetadataScheduler | Unset = FLUXDenoiseMetadataScheduler.EULER
    ...
    dype_preset: FLUXDenoiseMetadataDypePreset | Unset = FLUXDenoiseMetadataDypePreset.OFF
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        type_ = cast(Literal["flux_denoise_meta"], d.pop("type"))
        if type_ != "flux_denoise_meta":
            raise ValueError(f"type must match const 'flux_denoise_meta', got '{type_}'")
        ...
        scheduler: FLUXDenoiseMetadataScheduler | Unset
        if isinstance(_scheduler, Unset):
            scheduler = UNSET
        else:
            scheduler = FLUXDenoiseMetadataScheduler(_scheduler)
        ...
        dype_preset: FLUXDenoiseMetadataDypePreset | Unset
        if isinstance(_dype_preset, Unset):
            dype_preset = UNSET
        else:
            dype_preset = FLUXDenoiseMetadataDypePreset(_dype_preset)
        ...
        flux_denoise_metadata.additional_properties = d
        return flux_denoise_metadata
