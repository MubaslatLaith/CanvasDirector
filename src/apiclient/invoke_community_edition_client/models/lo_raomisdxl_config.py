...
T = TypeVar("T", bound="LoRAOMISDXLConfig")


@_attrs_define
class LoRAOMISDXLConfig:
    """
    Attributes:
        ...
        format_ (Literal['omi']):  Default: 'omi'.
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        format_ = cast(Literal["omi"], d.pop("format"))
        if format_ != "omi":
            raise ValueError(f"format must match const 'omi', got '{format_}'")

        base = cast(Literal["sdxl"], d.pop("base"))
        if base != "sdxl":
            raise ValueError(f"base must match const 'sdxl', got '{base}'")

        lo_raomisdxl_config = cls(
            key=key,
            hash_=hash_,
            path=path,
            file_size=file_size,
        ...
        lo_raomisdxl_config.additional_properties = d
        return lo_raomisdxl_config
